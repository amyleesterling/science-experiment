...
interface Props {
  ...
  // When false, OrbitControls are disabled so stray input can't hijack the
  // scripted camera — used by the /attract wall-display loop, which drives
...
export default function ZoomScene({
  ...
  useEffect(() => {
    ...
    // Wall-display / attract mode passes interactive=false so the scripted
    // camera can never be hijacked by stray input. Damping + update() still
    // run below; the scripted lerp drives everything.
    ...
    renderer.domElement.style.cursor = interactive ? "grab" : "none";
